...
## 1.1 当前进度快照（2026-04-01，已追加到本轮 TPS 交互修正）
...
- 已完成：
  ...
  - `Default / SceneObject / Ground` 避障层与输出相机手动防穿墙钳制已接入，镜头遇遮挡会向玩家方向回缩
  - 角色探索态下已可按移动方向驱动可见身体 / HitboxRig 朝向，瞄准态下已回正到相机 yaw
  - hip-fire 开火时角色会瞬时转向瞄准方向，并在射击窗口内保持第三人称射击朝向
  - 右键精确瞄准时已接入移动减速倍率
  ...
  - 持枪默认待机已切换到 ready / aim 上半身姿态，右键精瞄通过 `AimBlend`、机位和移速体现“更精确”
  ...
  - `PlayerInteractor` 已升级为 TPS 友好的交互查询：
    - 相机中心射线优先
    - 相机小半径容错 SphereCast
    - 玩家交互原点朝准星方向的 reach SphereCast
  - 交互查询距离已按肩后相机后撤距离补偿，避免“玩家眼前目标超出相机 3 米射线”的第三人称错位问题
  - 实心环境遮挡现在会阻断更远交互物，交互不再默认穿透墙体 / 柜体继续取后方目标
- 已知待补：
  ...
  - TPS 交互当前已解决“准星难对 / 相机后撤距离错位 / 隔墙穿透”三类核心问题，但具体容错半径和屏幕窗口仍需要后续手感微调
...
- 当镜头与 `Default / SceneObject / Ground` 遮挡发生冲突时，最终输出相机会向玩家方向压缩，避免直接穿墙。
...
- 角色移动速度下降，形成与 hip-fire 区分开的“更精确”射击态。
- 准星、武器散布、命中检测、枪口方向都以屏幕中心瞄准点为主，而不是纯相机正前方的第一人称假设。

### 3.3 hip-fire 射击态

- 持枪但未按右键时，角色允许继续按探索态移动，不强制持续盯住鼠标方向。
- 按下左键开火时，角色会立即吸附到当前相机 / 准星方向。
- 开火窗口内上半身保持射击朝向，下半身仍通过方向型 locomotion 承接移动。
- 这是当前资源条件下对“幻兽帕鲁式第三人称枪械手感”的桥接实现。
### 3.4 交互与动作态
...
- 管理越肩横向偏移
- 管理相机碰撞回缩
- 在必要时对最终输出相机再做一次手动 `SphereCast` 钳制，作为 Cinemachine 避障的兜底
...
- 处理骨骼缩放补偿、右手 socket 挂接和 world muzzle 兜底生成
...
- 交互射线建议改为 `PlayerAimPointResolver` 提供的瞄准方向，并增加 TPS 容错层：
  - 相机查询距离按肩后机位后撤距离补偿
  - 中心射线失败后允许小半径容错
  - 最终仍按玩家实际可触达距离过滤目标
  - 实心遮挡必须阻断更远交互体
